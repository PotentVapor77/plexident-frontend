...
import { Activity, Thermometer, Heart, Wind, Gauge, RefreshCw } from "lucide-react";
  ...
  return (
    <section className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden">
      ...
      <div className={`space-y-4 transition-opacity ${isRefreshing ? 'opacity-50' : 'opacity-100'}`}>
        {/* Mostramos un esqueleto si está refrescando */}
        {isRefreshing ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="p-4 rounded-lg border border-slate-200 bg-slate-50 animate-pulse">
