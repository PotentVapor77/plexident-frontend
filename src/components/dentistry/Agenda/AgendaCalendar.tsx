// AgendaCalendar.tsx - CORRECCIONES EN LAS FUNCIONES PRINCIPALES
import { useState, useRef, useEffect, useCallback } from "react";
import FullCalendar from "@fullcalendar/react";
import dayGridPlugin from "@fullcalendar/daygrid";
import timeGridPlugin from "@fullcalendar/timegrid";
import interactionPlugin from "@fullcalendar/interaction";
import { type DateSelectArg, type EventClickArg, type EventContentArg } from "@fullcalendar/core";
import { Modal } from "../../ui/modal";
import { useModal } from "../../../hooks/useModal";
import PageMeta from "../../common/PageMeta";
import esLocale from "@fullcalendar/core/locales/es";
import { useCitas } from "../../../hooks/useCitas";
import type { CalendarEvent, ICita } from "../../../types/ICita";
...
const AgendaCalendar: React.FC = () => {
  ...
  const formatTimeForDisplay = useCallback((dateString: string): string => {
    if (!dateString) return '';
    const date = new Date(dateString);
  ...
  // ✅ CORREGIDO: Convertir citas a eventos - Manejo correcto de IDs
  useEffect(() => {
    if (citas && citas.length > 0) {
      const calendarEvents: CalendarEvent[] = citas.map((cita: ICita) => {
        const color = getEventColor(cita.estado);
        
        const pacienteInfo = pacientes.find(p => p.id === cita.paciente);
        const odontologoInfo = odontologos.find(o => o.id === cita.odontologo);
        
        // Formatear nombres correctamente
        const pacienteNombre = pacienteInfo?.nombre || `Paciente ${cita.paciente}`;
        const odontologoNombre = odontologoInfo?.nombre || `Dr. ${cita.odontologo}`;
        const horaFormateada = formatTimeForDisplay(cita.fecha_inicio);

        // Título simple para el evento
        const eventTitle = `${pacienteNombre} - ${odontologoNombre}`;

        // ✅ CORREGIDO: Usar el ID correcto de la cita
        const citaId = cita.id || cita.id_cita;
        
        return {
          id: citaId?.toString() || Math.random().toString(),
          title: eventTitle,
  ...
  }, [citas, pacientes, odontologos, getEventColor, formatTimeForDisplay]);
  ...
  const handleEventClick = useCallback((clickInfo: EventClickArg) => {
    ...
    const citaData = event.extendedProps.citaData as ICita;
    
    loadCitaData(citaData);
    openModal();
  }, [loadCitaData, openModal]);

  // ✅ FUNCIÓN CRÍTICAMENTE CORREGIDA: Manejo correcto de creación vs actualización
  const handleAddOrUpdateEvent = useCallback(async () => {
    try {
      ...
      // ✅ CORREGIDO: Lógica mejorada para determinar si es creación o actualización
      if (selectedCita) {
        // Usar el ID correcto (id o id_cita)
        const citaId = selectedCita.id || selectedCita.id_cita;
        if (citaId) {
          console.log('🔄 Actualizando cita existente con ID:', citaId);
          success = await handleUpdateCita(citaId, citaData);
        } else {
          console.log('🆕 Creando nueva cita (no hay ID válido)');
  ...
  const handleDeleteEvent = useCallback(async () => {
    if (selectedCita) {
      ...
      const citaId = selectedCita.id || selectedCita.id_cita;
      if (citaId && window.confirm('¿Está seguro de que desea eliminar esta cita?')) {
        try {
          const success = await handleDeleteCita(citaId);
          if (success) {
            closeModal();
            resetCitaForm();
          }
  ...
  return (
    <>
        ...
        >
          <div className="bg-white dark:bg-gray-800">
            ...
            <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
              {/* Información de la cita en el formato solicitado */}
              {selectedCita && (
                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                  <div className="text-center space-y-2">
                    <div className="text-lg font-semibold text-blue-800 dark:text-blue-300">
                      {formatTimeForDisplay(selectedCita.fecha_inicio)}
                    </div>
                    <div className="text-md font-medium text-gray-800 dark:text-white">
                      {pacientes.find(p => p.id === selectedCita.paciente)?.nombre || `Paciente ${selectedCita.paciente}`}
                    </div>
                    <div className="text-sm text-gray-600 dark:text-gray-300">
                      {odontologos.find(o => o.id === selectedCita.odontologo)?.nombre || `Dr. ${selectedCita.odontologo}`}
                    </div>
                  </div>
                </div>
              )}
      ...
      <style>{`
        ...
        /* Estilos para mejorar la visualización de eventos */
        .fc-daygrid-event {
          margin: 1px 2px;
        }
        ...
        /* Asegurar que el contenido del evento tenga suficiente espacio */
...
// Componente CORREGIDO para renderizar el contenido del evento
