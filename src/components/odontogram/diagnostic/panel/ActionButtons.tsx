...
export const ActionButtons: React.FC<ActionButtonsProps> = ({
    ...
    const handleClearTooth = async () => {
        ...
        try {
            // Recolectar IDs únicos de diagnósticos del diente
            const ids = new Set<string>();
            for (const diag of diagnosticosAplicados) {
                for (const { id } of diag.diagnosticoIds) {
                    ids.add(id);
                }
            }
            ...
            onClearAll(); // limpia el odontogramaData del diente

            addNotification({
  type: 'success',
  title: 'Diente limpiado',
  message: `Se eliminaron ${diagnosticosCount} diagnóstico(s) del diente.`,
});
        } catch (error) {
  ...
  message: 'No se pudieron eliminar los diagnósticos. Intenta nuevamente.',
});
        }
    };
