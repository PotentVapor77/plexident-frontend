// src/components/personalBackground/table/PersonalBackgroundTable.tsx
...
import type { IPersonalBackground, IPacienteBasico } from "../../../../types/personalBackground/IPersonalBackground";
import {
  tieneAntecedentesCriticos,
  contarAntecedentesActivos,
} from "../../../../types/personalBackground/IPersonalBackground";

interface PersonalBackgroundTableProps {
  onEdit?: (background: IPersonalBackground) => void;
  onView?: (background: IPersonalBackground) => void;
  onDelete?: (background: IPersonalBackground) => void;
}
...
export function PersonalBackgroundTable({
  onEdit,
  onView,
  onDelete,
}: PersonalBackgroundTableProps) {
  ...
  const { data: pacientesData, isLoading: isLoadingPacientes } = useQuery({
    queryKey: ['pacientesForBackgrounds', pacienteIds],
    queryFn: async () => {
  ...
  const handleView = (background: IPersonalBackground) => {
    onView?.(background);
  };

  const handleEdit = (background: IPersonalBackground) => {
    onEdit?.(background);
  };

  const handleDeleteClick = (background: IPersonalBackground) => {
    onDelete?.(background);
  };

  // ✅ Función para obtener el objeto paciente (si es string, busca en cache)
  const getPacienteObject = (background: IPersonalBackground): IPacienteBasico | null => {
  ...
  const getPatientName = (background: IPersonalBackground): string => {
    const pacienteObj = getPacienteObject(background);
    if (pacienteObj) {
      return `${pacienteObj.nombres || ''} ${pacienteObj.apellidos || ''}`.trim() || 'Paciente';
    }
    return 'Paciente';
  };

  // ✅ Función para obtener iniciales del paciente
  const getPatientInitials = (background: IPersonalBackground): string => {
    const pacienteObj = getPacienteObject(background);
    if (pacienteObj) {
      const firstInitial = pacienteObj.nombres?.charAt(0)?.toUpperCase() || 'P';
      const lastInitial = pacienteObj.apellidos?.charAt(0)?.toUpperCase() || '';
  ...
  const getPatientId = (background: IPersonalBackground): string => {
    const pacienteObj = getPacienteObject(background);
    if (pacienteObj) {
      return pacienteObj.cedula_pasaporte || pacienteObj.id.substring(0, 8) || 'N/A';
    }
          ...
          <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
            {backgroundsList.length === 0 ? (
              ...
              backgroundsList.map((background) => {
                ...
                if (!pacienteObj && typeof background.paciente === 'string') {
                  return (
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        {tieneCriticos && (
                          <span className="text-red-600" title="Antecedentes críticos">
                            ⚠️
                          </span>
                        )}
                        <div className="text-sm text-gray-700 dark:text-gray-300">
                          {totalAntecedentes > 0 ? (
                            <>
                              <span className="font-medium">{totalAntecedentes}</span>{" "}
                              antecedente{totalAntecedentes !== 1 && "s"}
                            </>
                          ) : (
                            <span className="text-gray-400">Sin antecedentes</span>
                          )}
                        </div>
                      </div>
                    </td>
                      ...
                      <div className="flex justify-end gap-2">
                        ...
                        {onDelete && (
                              <button
                                ...
                                title={!background.activo ? "Antecedentes inactivos no se pueden eliminar" : "Eliminar"}
                              >
                                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
