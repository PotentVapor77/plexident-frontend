...
const fdiToMeshKey: Record<string, string> = {};
Object.entries(toothTranslations).forEach(([meshKey, data]) => {
  const fdi = String(data.numero);
  if (!fdiToMeshKey[fdi]) {
    fdiToMeshKey[fdi] = meshKey;
  }
});


// Obtener traducción a partir del número FDI
export const getToothTranslationByFdi = (
  fdi: string
): { nombre: string; numero: number } => {
  const meshKey = fdiToMeshKey[fdi];
  if (!meshKey) {
    return { nombre: `Diente #${fdi}`, numero: Number(fdi) || 0 };
  }
  return toothTranslations[meshKey];
};

export const getToothSpanishNameByFdi = (fdi: string): string => {
  return getToothTranslationByFdi(fdi).nombre;
};

export const getToothNumberByFdi = (fdi: string): number => {
  return getToothTranslationByFdi(fdi).numero;
};

export const fdiToMeshName = (fdi: string): string | null => {
  return fdiToMeshKey[fdi] || null;
};
